...
class ArvoreBinaria {
  ...
  atualizarProduto(nomeProduto, descricao, preco, categoria) {
      this.raiz = this.atualizarProdutoRecursivo(this.raiz, nomeProduto, descricao, preco, categoria);
      localStorage.setItem('arvoreBinaria', JSON.stringify(this));
      alert("O produto foi atualizado com sucesso!");
      redirecionarParaPagina('ListaProdutos.html');
  }

  atualizarProdutoRecursivo(no, nomeProduto, descricao, preco, categoria) {
      if (no === null) {
          return null;
      }

      if (nomeProduto < no.produto.nome) {
          no.esquerda = this.atualizarProdutoRecursivo(no.esquerda, nomeProduto, descricao, preco, categoria);
      } else if (nomeProduto > no.produto.nome) {
          no.direita = this.atualizarProdutoRecursivo(no.direita, nomeProduto, descricao, preco, categoria);
      } else {
          no.produto.descricao = descricao;
          no.produto.preco = preco;
          no.produto.categoria = categoria;
      }
